...
class BrainFloatQuantizer(torch.nn.Module):
    def __init__(self, fp_exp, fp_man, forward_rounding="nearest"):
        super().__init__()
        self.fp_exp = fp_exp
        self.fp_man = fp_man
...
class BfpQuantizer(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        reshaped_input = torch.reshape(transp_input, (-1, transp_input.shape[-1]))

        # Pad input if necessary
        if reshaped_input.shape[-1] % self.block_size > 0:
            # Pad over the `dim` dimension
            pad = self.block_size - (reshaped_input.shape[-1] % self.block_size)
            ...
            pad_tensor = min_values.unsqueeze(1).repeat(1, pad).to(input.device)
