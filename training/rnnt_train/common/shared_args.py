...
def add_shared_args(parser: ArgumentParser) -> None:
    parser.add_argument(
        ...
        action="store_true",
        help="""Normally the start of the audio is padded with (window_size - window_stride)
        seconds of silence to match the server's behavior. This option turns that off.""",
    )
    parser.add_argument(
        "--prob_val_narrowband",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Read data from directories instead of json manifest files",
    )
    parser.add_argument(
        "--val_audio_dir",
        type=str,
