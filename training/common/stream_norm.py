...
# Written Jun 2022 by rob@myrtle with thanks to jason@myrtle.
...
class StreamNorm:
    def __init__(self, alpha, mel_means, mel_vars):
        ...
        # mel_means should be a vector of floats, shape (mel_dim,), a PyTorch Tensor or numpy array
        self.mel_means = mel_means

        # mel_vars should be a vector of positive floats, shape (mel_dim,), a PyTorch Tensor or numpy array
        z = np.zeros_like(mel_vars)
        np.testing.assert_array_less(z, mel_vars, '\nERROR : All variances should be positive\n')
        self.mel_vars  = mel_vars

    def normalize(self, new_vec):   # vector of floats, shape (mel_dim,), PyTorch Tensor or numpy array
        """
        Formulae from:
        ...
        https://silo.tips/download/incremental-calculation-of-weighted-mean-and-variance (end of)
        """
        # Update the internal means and vars with the new vector using the formulae just mentioned.
        # Note that since mel_vars is always updated by the old_vars plus a squared quantity, and
        # since the old_vars begin positive (asserted above) and since 0<=alpha<1 (asserted above),
        # the new vars are guaranteed positive in all circumstances, so the sqrt will never fail.
