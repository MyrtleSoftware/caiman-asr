...
class DaliDataLoader:
    ...
    def _init_iterator(self, gpu_id, dataset_path, config_data, config_features, json_names: list, tokenizer: list,
        ...
        pipeline = DaliPipeline.from_config(config_data=config_data, config_features=config_features,
                                            normalize=normalize, num_cpu_threads=num_cpu_threads, device_id=gpu_id,
                                            file_root=dataset_path, sampler=self.sampler,
                                            device_type=self.device_type, batch_size=self.batch_size,
                                            pipeline_type=pipeline_type)

        return DaliRnntIterator([pipeline], transcripts=transcripts, tokenizer=tokenizer,
                                batch_size=self.batch_size, shard_size=self._shard_size(),
                                pipeline_type=pipeline_type, device_type=self.device_type)
