...
from common.hard_activation_functions import Hardsigmoid, Hardtanh
from common.quantize import BrainFloatQuantizer, BfpQuantizer, NullClass
...
class CustomLSTM(nn.Module):
    ...
    """
    def __init__(self, input_size, hidden_size, num_layers=1, dropout=0.0, hard=False, quantize=False):
        super().__init__()
        ...
        if self.quantize:
            ...
            self.custom_layer = torch.jit.script(CustomLSTMLayer(hidden_size, hard, quantize))
    ...
    def forward(self, input_tensor, init_states=None):
        ...
        for layer in range(self.num_layers):
            ...
            U   = getattr(self, f"weight_ih_l{layer}") # (4*hidden_size, layer_input_size)
            ...
            output, (h_f, c_f) = self.custom_layer(layer_input, h_0, c_0, U, V, bih, bhh)
...
class CustomLSTMLayer(nn.Module):
    ...
    def __init__(self, hidden_size, hard, quantize):
        ...
        if self.quantize:
            self.bf16 = BrainFloatQuantizer(fp_exp=8, fp_man=7, forward_rounding="nearest")
            self.bfp_dim0 = BfpQuantizer(dim=0, block_size=8, fp_exp=8, fp_man=7, forward_rounding="nearest")
            self.bfp_dim1 = BfpQuantizer(dim=1, block_size=8, fp_exp=8, fp_man=7, forward_rounding="nearest")
        else:
            self.bf16 = NullClass()
