...
well as training data meaning that dev-set WERs may be lower that they might be for the full character set. By default, the `configs/` 
set `normalize_transcripts: true` meaning that, even if transcripts aren't normalized during this preprocessing stage, they will be 
normalized on the fly during training **and validation** with the [_clean_text](./common/text/__init__.py) function.
...
For training experiments with 1k to 10k hrs of data we find that it is best to keep `WARMUP_STEPS` and `HOLD_STEPS` at their default values.
However, the best value of `HALF_LIFE_STEPS` will change depending on the amount of data you are using. Specifically, we
...
Training the RNN-T system with a large tokenizer (ie. much bigger than 1023 tokens) often results in exploding gradients
...
```
DUMP_MEL_STATS=true NUM_GPUS=2 GRAD_ACCUMULATION_BATCHES=21 EPOCHS=1 ./scripts/train.sh
```
