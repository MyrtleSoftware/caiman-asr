...
def stdout_step_format(step):
    ...
    if len(step) > 3:
        fields.append("step {:>4}".format(step[3]))
    return " | ".join(fields)
...
def init_log(args):
    ...
    for id_, pref in [
        ...
        dllogger.metadata(f"{id_}_loss", {"name": f"{pref}loss", "format": ":>7.2f"})

        dllogger.metadata(f"{id_}_wer", {"name": f"{pref}wer", "format": ":>6.2f"})

        dllogger.metadata(f"{id_}_pplx", {"name": f"{pref}pplx", "format": ":>6.2f"})

        dllogger.metadata(
            f"{id_}_throughput", {"name": f"{pref}utts/s", "format": ":>5.0f"}
        )
